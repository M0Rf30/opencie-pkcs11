 ...
 *   2. CIE_SetNFCTag(env, isoDepObject)   →  stores IsoDep global ref
 *   3. C_Initialize(NULL)                  →  creates AndroidNFCTransport
 *   4. ... normal PKCS#11 usage ...
 *   5. C_Finalize(NULL)
 *   6. CIE_ClearNFCTag(env)               →  releases IsoDep global ref
 */
#pragma once
...
void CIE_SetNFCTag(JNIEnv *env, jobject isoDep);

/* Clear the IsoDep reference (call when NFC tag is lost). */
void CIE_ClearNFCTag(JNIEnv *env);
}
