...
extern "C" __attribute__((visibility("default"))) void cie_set_nfc_tag(
    JNIEnv *env, jobject isoDep) {
  ...
  if (isoDep) {
    ...
    LOGI("cie_set_nfc_tag: IsoDep reference stored");
  }
}

extern "C" __attribute__((visibility("default"))) void cie_clear_nfc_tag(
    JNIEnv *env) {
  std::lock_guard<std::mutex> lock(g_globalMutex);
  if (g_isoDep) {
    env->DeleteGlobalRef(g_isoDep);
    g_isoDep = nullptr;
    LOGI("cie_clear_nfc_tag: IsoDep reference cleared");
  }
}

/* ===== JNI-named wrappers for the OpenCIE Flutter app =====
 *
 ...
 */
extern "C" void cie_set_data_dir(const char *dir);
...
AndroidNFCTransport::AndroidNFCTransport()
  ...
  if (!isoDep_) {
    LOGE("AndroidNFCTransport: no IsoDep set (call cie_set_nfc_tag first)");
    return;
  }
