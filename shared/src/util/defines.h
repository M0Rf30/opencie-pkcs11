...
/**
 * @file defines.h
 * @brief Preprocessor macros, smart card error codes, and logging helpers.
 *
 * Defines ISO 7816 status word constants for CIE smart card error handling,
 * parameter logging macros, and the ER_ASSERT assertion macro.
 */

#pragma once

#include "func_call_info.h"

/** @brief Type alias for ISO 7816 status words (SW1-SW2). */
#define StatusWord uint16_t

/** @name Smart Card Error Status Words
 *  ISO 7816 status word constants returned by the CIE card.
 *  @{
 */
#define ERR_CARD_FILE_DEACTIVATED 0x6283
#define ERR_CARD_FILE_TERMINATED 0x6285
#define ERR_CARD_AUTHENTICATION_FAILED 0x6300
...
/** @name Logging Macros
 *  Placeholder macros for parameter and binary data logging.
 *  @{
 */
#define logParam(p)
...
/** @brief Placeholder for function exit tracing (currently a no-op). */
#define exit_func

/**
 * @brief Assert a condition and throw a logged_error on failure.
