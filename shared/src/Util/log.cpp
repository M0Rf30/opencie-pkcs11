...
void initLog(const char *moduleName, const char * /*iniFile*/, const char *version) {
  ...
  LogMode = static_cast<logMode>(settings.getIntProperty("LogMode", static_cast<int>(LM_Single)));
...
DWORD CLog::write(const char *format, ...) {
  ...
  if (Enabled && Initialized && mainEnable) {
    ...
    struct tm t;
    struct tm tm = *localtime_r(&T, &t);
    snprintf(pbtDate, 20, "%05u:[%02d:%02d:0%02d]", *Num, tm.tm_hour,
             tm.tm_min, tm.tm_sec);
#endif
    ...
    if (lf) {
      switch (LogMode) {
        case (LM_Single):
          fprintf(lf, "%s|%04lu|%04lx|%02i|", pbtDate, (unsigned long)GetCurrentProcessId(),
                  (unsigned long)dwThreadID, ModuleNum);
          break;
        case (LM_Module):
          fprintf(lf, "%s|%04lu|%04lx|", pbtDate, (unsigned long)GetCurrentProcessId(),
                  (unsigned long)dwThreadID);
          break;
        case (LM_Thread):
          fprintf(lf, "%s|%04lu|%02i|", pbtDate, (unsigned long)GetCurrentProcessId(),
                  ModuleNum);
          break;
        case (LM_Module_Thread):
    ...
    if (lf) {
      ...
      switch (LogMode) {
        case (LM_Single):
          fprintf(lf, "%s|%04i|%04lx|%02i|", pbtDate, getpid(),
                  dwThreadID, ModuleNum);
          break;
        case (LM_Module):
