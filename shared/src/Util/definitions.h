...
#ifndef LOWORD
constexpr uint16_t LOWORD(uint32_t l) { return static_cast<uint16_t>(l & 0xFFFF); }
constexpr uint16_t HIWORD(uint32_t l) { return static_cast<uint16_t>((l >> 16) & 0xFFFF); }
#endif

#ifndef LOBYTE
constexpr uint8_t LOBYTE(uint32_t l) { return static_cast<uint8_t>(l & 0xFF); }
constexpr uint8_t HIBYTE(uint32_t l) { return static_cast<uint8_t>((l >> 8) & 0xFF); }
#endif
...
#endif
constexpr uint16_t MAKEWORD(uint8_t lo, uint8_t hi) { return static_cast<uint16_t>(lo + (hi * 256)); }

void SetLastError(unsigned long nErr);
unsigned long GetLastError();
#endif  // !_WIN32

constexpr const char* szIdAASigningCertificateV2OID = "1.2.840.113549.1.9.16.2.47";
