...
UUCByteArray::UUCByteArray(const BYTE* pbtContent, const unsigned long unLen)
  ...
  if (m_unLen > 0) {
    m_nCapacity = m_unLen;
    m_pbtContent = static_cast<BYTE*>(malloc(m_nCapacity));
    if (m_pbtContent == nullptr) throw -5L;
...
UUCByteArray::UUCByteArray(const UUCByteArray& blob) : m_szHex(nullptr) {
...
UUCByteArray::UUCByteArray(const char* szHexString) : m_szHex(nullptr) {
  ...
  for (unsigned int i = 0; i < m_unLen; i++) {
    m_pbtContent[i] = atox(const_cast<char*>(szHexString) + (i * 2));
  }
}

UUCByteArray::UUCByteArray(const unsigned long nLen) : m_szHex(nullptr) {
  // m_unLen = nLen;
  m_unLen = 0;
  m_nCapacity = nLen;
  // m_pbtContent = (BYTE*)GlobalAlloc(GPTR, m_nCapacity);
...
UUCByteArray::UUCByteArray() : m_szHex(nullptr) {
  m_unLen = 0;
  m_nCapacity = DEFAULT_CAPACITY;
  // m_pbtContent = (BYTE*)GlobalAlloc(GPTR, m_nCapacity);
  m_pbtContent = static_cast<BYTE*>(malloc(m_nCapacity));
...
long UUCByteArray::load(const char* szHexString) {
  ...
  // m_pbtContent = (BYTE*)GlobalAlloc(GPTR, m_nCapacity);
  m_pbtContent = static_cast<BYTE*>(malloc(m_nCapacity));
  if (m_pbtContent == nullptr) return ERROR_UNABLE_TO_ALLOCATE;
...
long UUCByteArray::append(const BYTE btVal) {
  if (m_unLen == m_nCapacity) {
    m_nCapacity += DEFAULT_CAPACITY;
    BYTE* buffer = static_cast<BYTE*>(realloc(m_pbtContent, m_nCapacity));
    if (buffer == nullptr) return ERROR_UNABLE_TO_ALLOCATE;
...
void UUCByteArray::toHexString(char* szHexString) const
{
        try
        {
                ...
                for(unsigned int i = 0; i < m_unLen; i++)
                {
                        snprintf(szDigit, sizeof(szDigit), "%02X", m_pbtContent[i]);
...
long UUCByteArray::reverse() {
  BYTE* pbtContent = static_cast<BYTE*>(malloc(m_unLen));
  if (pbtContent == nullptr) return ERROR_UNABLE_TO_ALLOCATE;

  for (uint64_t i = 0; i < m_unLen; i++) {
...
const char* UUCByteArray::toHexString(int nSize) {
  ...
  try {
    for (unsigned int i = 0; i < static_cast<unsigned int>(nSize); i++) {
      snprintf(m_szHex + i * 2, 3, "%02X", m_pbtContent[i]);
    }
    m_szHex[nSize * 2] = '\0';
