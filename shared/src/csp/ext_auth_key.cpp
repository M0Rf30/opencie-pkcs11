// SPDX-FileCopyrightText: 2021 Gianluca Boiano
// SPDX-License-Identifier: GPL-2.0-or-later

/**
 * @file ext_auth_key.cpp
 * @brief External authentication RSA private exponent for CIE card mutual
 *        authentication.
 *
 * Contains the RSA private exponent used during the external authentication
 * step of the IAS ECC protocol. This key is part of the challenge-response
 * mechanism that allows the host application to authenticate itself to the
 * CIE smart card.
 */

#include "util/array.h"
#include "util/definitions.h"

/** @brief RSA private exponent byte array for external authentication. */
BYTE ExtAuth_PrivExp[] = {
    };

/** @brief ByteArray wrapper around ExtAuth_PrivExp for use in the IAS protocol.
 */
ByteArray baExtAuth_PrivExp(ExtAuth_PrivExp, sizeof(ExtAuth_PrivExp));
