...
static HRESULT TokenTransmitCallback(void *data, BYTE *apdu, DWORD apduSize, BYTE *resp,
                              DWORD *respSize) {
  ...
  if (apduSize == 2) {
    ...
    if (code == 0xfffd) {
      *respSize = sizeof(slot->hCard) + 2;
      ...
      DWORD protocol = 0;
      auto ris =
          slot->transport.Reconnect(slot->hCard, SCARD_SHARE_SHARED, SCARD_PROTOCOL_Tx,
  ...
  if (ris == SCARD_W_RESET_CARD || ris == SCARD_W_UNPOWERED_CARD) {
    ...
    DWORD protocol = 0;
    ris = slot->transport.Reconnect(slot->hCard, SCARD_SHARE_SHARED, SCARD_PROTOCOL_Tx,
                         SCARD_LEAVE_CARD, &protocol);
    if (ris != SCARD_S_SUCCESS) {
      LOG_ERROR("TokenTransmitCallback - Errore reconnect %d", ris);
    } else {
      ris = slot->transport.Transmit(slot->hCard, SCARD_PCI_T1, apdu, apduSize, resp,
                          respSize);
    }
  }
...
void CIEtemplateInitSession(void *pTemplateData) {
  ...
  if (!cie->init) {
    ...
    VarToByteArray(start.year).copy(ByteArray(reinterpret_cast<BYTE *>(sFrom), 4));
    VarToByteArray(start.month).copy(ByteArray(reinterpret_cast<BYTE *>(&sFrom[4]), 2));
    VarToByteArray(start.day).copy(ByteArray(reinterpret_cast<BYTE *>(&sFrom[6]), 2));

    VarToByteArray(end.year).copy(ByteArray(reinterpret_cast<BYTE *>(sTo), 4));
    VarToByteArray(end.month).copy(ByteArray(reinterpret_cast<BYTE *>(&sTo[4]), 2));
    VarToByteArray(end.day).copy(ByteArray(reinterpret_cast<BYTE *>(&sTo[6]), 2));
