...
/**
 * @file pkcs11_functions.cpp
 * @brief PKCS#11 function implementations and module entry point
 */
...
template <typename F>
CK_RV pkcs11_guard(const char *funcName, F &&func) {
  LOG_INFO("[PKCS11] %s", funcName);
  try {
    return func();
    ...
    LOG_ERROR("%s, CKR_GENERAL_ERROR", funcName);
    return CKR_GENERAL_ERROR;
  }
}
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved) {
  switch (fdwReason) {
    case DLL_PROCESS_ATTACH: {
      ...
      if (!GetEnvironmentVariableA("USERPROFILE", homedir, MAX_PATH)) {
        char homedrive[MAX_PATH] = {0};
        char homepath[MAX_PATH] = {0};
        GetEnvironmentVariableA("HOMEDRIVE", homedrive, MAX_PATH);
        GetEnvironmentVariableA("HOMEPATH", homepath, MAX_PATH);
        homeStr = std::string(homedrive) + homepath;
      } else {
        homeStr = homedir;
      }
      bModuleInit = true;
      std::string configPath = homeStr + "/.CIEPKI/ciepki.ini";
      initLog("CIEPKC11", configPath.c_str(), __DATE__ " " __TIME__);
      break;
    }
    case DLL_PROCESS_DETACH:
      // NSS calls C_Finalize() before unloading the module.
      // Calling it again here under the loader lock can deadlock
      // or crash (double-finalization, threads already dead).
      bModuleInit = false;
      bP11Terminate = true;
      break;
  }
...
__attribute__((destructor)) void DllMainDetach() {
  ...
  if (bP11Initialized) {
    LOG_INFO("[PKCS11] DllMainDetach - Force C_Finalize");
    C_Finalize(nullptr);
...
CK_RV CK_ENTRY C_GetSlotList(CK_BBOOL tokenPresent, CK_SLOT_ID_PTR pSlotList,
  ...
  return pkcs11_guard(__FUNCTION__, [&]() -> CK_RV {
    ...
    if (tokenPresent) {
      ...
      for (auto it = CSlot::g_mSlots.rbegin(); it != CSlot::g_mSlots.rend();
           ++it) {
      ...
      // return all readers
      if (!pSlotList) {
        iCnt = static_cast<CK_ULONG>(CSlot::g_mSlots.size());
      } else {
        iCnt = 0;
        for (auto it = CSlot::g_mSlots.rbegin(); it != CSlot::g_mSlots.rend();
             ++it) {
...
CK_RV CK_ENTRY C_Initialize(CK_VOID_PTR pReserved) {
  return pkcs11_guard(__FUNCTION__, [&]() -> CK_RV {
    ...
    // verify that the flags are supported
    CK_C_INITIALIZE_ARGS_PTR iargs = nullptr;
    if (pReserved) {
      iargs = static_cast<CK_C_INITIALIZE_ARGS_PTR>(pReserved);
      if (iargs->pReserved != nullptr) throw p11_error(CKR_ARGUMENTS_BAD);

      if (iargs->flags & CKF_OS_LOCKING_OK) {
        // Nothing to do because we will use os locking
      } else if (iargs->flags == 0) {
        if ((iargs->CreateMutex) || (iargs->DestroyMutex) ||
            (iargs->LockMutex) || (iargs->UnlockMutex))
...
CK_RV CK_ENTRY C_Finalize(CK_VOID_PTR pReserved) {
  return pkcs11_guard(__FUNCTION__, [&]() -> CK_RV {
    ...
    logParam(pReserved)
        ...
        if (pReserved != nullptr) throw p11_error(CKR_ARGUMENTS_BAD);
    ...
    if (CSlot::Thread.joinable()) {
      CCardContext *tc = CSlot::ThreadContext.load();
      if (tc != nullptr) {
...
CK_RV CK_ENTRY C_OpenSession(CK_SLOT_ID slotID, CK_FLAGS flags,
  ...
  return pkcs11_guard(__FUNCTION__, [&]() -> CK_RV {
    ...
    auto pSession = std::make_unique<CSession>();
    ...
    if ((flags & CKF_RW_SESSION) == 0) {
      // if a RO session, must
      // verify there are no SO RW sessions
...
CK_RV CK_ENTRY C_GetFunctionList(CK_FUNCTION_LIST_PTR_PTR ppFunctionList) {
  return pkcs11_guard(__FUNCTION__, [&]() -> CK_RV {
    ...
    logParam(ppFunctionList)
        ...
        if (ppFunctionList == nullptr) throw p11_error(CKR_ARGUMENTS_BAD);

    static CK_FUNCTION_LIST functionList = {{2, 40},
                                            C_Initialize,
                                            C_Finalize,
                                            C_GetInfo,
                                            C_GetFunctionList,
...
CK_RV CK_ENTRY C_FindObjects(CK_SESSION_HANDLE hSession,
  ...
  return pkcs11_guard(__FUNCTION__, [&]() -> CK_RV {
    ...
    if (phObject == nullptr && ulMaxObjectCount > 0)
      throw p11_error(CKR_ARGUMENTS_BAD);
...
CK_RV CK_ENTRY C_GetMechanismList(CK_SLOT_ID slotID,
  ...
  return pkcs11_guard(__FUNCTION__, [&]() -> CK_RV {
    ...
    logParam(slotID) logParam(pMechanismList) logParam(pulCount)
        ...
        DWORD dwNumMechansms =
            sizeof(p11_mechanisms) / sizeof(CK_MECHANISM_TYPE);
    ...
    if (*pulCount >= dwNumMechansms) {
      std::memcpy(pMechanismList, p11_mechanisms,
                  dwNumMechansms * sizeof(CK_MECHANISM_TYPE));
      return CKR_OK;
    } else
...
CK_RV CK_ENTRY C_Sign(CK_SESSION_HANDLE hSession, CK_BYTE_PTR pData,
  ...
  return pkcs11_guard(__FUNCTION__, [&]() -> CK_RV {
    ...
    *pulSignatureLen = static_cast<CK_ULONG>(Signature.size());
    return CKR_OK;
  });
}
...
CK_RV CK_ENTRY C_VerifyRecover(CK_SESSION_HANDLE hSession,
  ...
  return pkcs11_guard(__FUNCTION__, [&]() -> CK_RV {
    ...
    *pulDataLen = static_cast<CK_ULONG>(Data.size());
...
CK_RV CK_ENTRY C_Encrypt(CK_SESSION_HANDLE /*hSession*/, CK_BYTE_PTR /*pData*/,
  ...
  return pkcs11_guard(__FUNCTION__,
                      [&]() -> CK_RV { return CKR_FUNCTION_NOT_SUPPORTED; });
}
...
CK_RV CK_ENTRY C_WaitForSlotEvent(CK_FLAGS flags, CK_SLOT_ID_PTR pSlot,
  ...
  return pkcs11_guard(__FUNCTION__, [&]() -> CK_RV {
    ...
    if (flags & CKF_DONT_BLOCK) {
      std::unique_lock<std::mutex> lock(p11Mutex);
      for (auto &[id, slot] : CSlot::g_mSlots) {
        if (slot->lastEvent != SlotEvent::NoEvent) {
          *pSlot = slot->hSlot;
          slot->lastEvent = SlotEvent::NoEvent;
          return CKR_OK;
        }
      }
...
CK_RV CK_ENTRY C_GetOperationState(CK_SESSION_HANDLE hSession,
  ...
  return pkcs11_guard(__FUNCTION__, [&]() -> CK_RV {
    ...
    *pulOperationStateLen = static_cast<CK_ULONG>(OperationState.size());
...
unsupported CK_RV CK_ENTRY C_CopyObject(CK_SESSION_HANDLE /*hSession*/,
                                        CK_OBJECT_HANDLE /*hObject*/,
                                        CK_ATTRIBUTE_PTR /*pTemplate*/,
                                        CK_ULONG /*ulCount*/,
                                        CK_OBJECT_HANDLE_PTR /*phNewObject*/)
unsupported CK_RV CK_ENTRY C_DigestKey(CK_SESSION_HANDLE /*hSession*/,
                                       CK_OBJECT_HANDLE /*hKey*/)
unsupported CK_RV CK_ENTRY
C_DigestEncryptUpdate(CK_SESSION_HANDLE /*hSession*/, CK_BYTE_PTR /*pPart*/,
                      CK_ULONG /*ulPartLen*/, CK_BYTE_PTR /*pEncryptedPart*/,
                      CK_ULONG_PTR /*pulEncryptedPartLen*/)
unsupported CK_RV CK_ENTRY C_DecryptDigestUpdate(
...
C_SignEncryptUpdate(CK_SESSION_HANDLE /*hSession*/, CK_BYTE_PTR /*pPart*/,
                    CK_ULONG /*ulPartLen*/, CK_BYTE_PTR /*pEncryptedPart*/,
                    CK_ULONG_PTR /*pulEncryptedPartLen*/)
unsupported CK_RV CK_ENTRY C_DecryptVerifyUpdate(
    CK_SESSION_HANDLE /*hSession*/, CK_BYTE_PTR /*pEncryptedPart*/,
    CK_ULONG /*ulEncryptedPartLen*/, CK_BYTE_PTR /*pPart*/,
    CK_ULONG_PTR /*pulPartLen*/)
unsupported CK_RV CK_ENTRY C_WrapKey(CK_SESSION_HANDLE /*hSession*/,
                                     CK_MECHANISM_PTR /*pMechanism*/,
...
unsupported CK_RV CK_ENTRY C_DeriveKey(CK_SESSION_HANDLE /*hSession*/,
                                       CK_MECHANISM_PTR /*pMechanism*/,
                                       CK_OBJECT_HANDLE /*hBaseKey*/,
                                       CK_ATTRIBUTE_PTR /*pTemplate*/,
                                       CK_ULONG /*ulAttributeCount*/,
                                       CK_OBJECT_HANDLE_PTR /*phKey*/)
unsupported CK_RV CK_ENTRY C_GetFunctionStatus(CK_SESSION_HANDLE /*hSession*/)
unsupported CK_RV CK_ENTRY C_CancelFunction(CK_SESSION_HANDLE /*hSession*/)
unsupported

    const char *
    getAttributeName(unsigned long dwId) {
  switch (dwId) {
    case 0x00000000:
      return ("CKA_CLASS");
    case 0x00000001:
