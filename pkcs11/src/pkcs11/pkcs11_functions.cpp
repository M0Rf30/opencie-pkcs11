      ...
      switch (pTemplate[i].type) {
        ...
        case CKA_MODIFIABLE:
          LOG_DEBUG(
              "[PKCS11] WriteAttributes - %d) type=%x (%s), value=%x, len=%x",
              i + 1, pTemplate[i].type, getAttributeName(pTemplate[i].type),
              *static_cast<CK_BBOOL*>(pTemplate[i].pValue), pTemplate[i].ulValueLen);
          break;
        ...
        case CKA_OBJECT_ID:
          LOG_DEBUG(
              ...
              i + 1, pTemplate[i].type, getAttributeName(pTemplate[i].type),
              static_cast<char*>(pTemplate[i].pValue), pTemplate[i].ulValueLen);
          break;
        ...
        case CKA_VALUE:
          LOG_DEBUG(
              "[PKCS11] WriteAttributes - %d) type=%x (%s), value=%s, len=%x",
              i + 1, pTemplate[i].type, getAttributeName(pTemplate[i].type),
              UUCByteArray(static_cast<BYTE*>(pTemplate[i].pValue), pTemplate[i].ulValueLen)
                  .toHexString(),
              pTemplate[i].ulValueLen);
          break;
...
CK_RV CK_ENTRY C_GetSlotList(CK_BBOOL tokenPresent, CK_SLOT_ID_PTR pSlotList,
  ...
  if (tokenPresent) {
    ...
    for (auto it = CSlot::g_mSlots.rbegin(); it != CSlot::g_mSlots.rend(); ++it) {
