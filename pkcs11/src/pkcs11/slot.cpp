...
CSlot::CSlot(ISmartCardTransport &transport, const char *szReader) : transport(transport), Context(transport) {
  szName = szReader;
  ...
  while (true) {
    ...
    LONG ris;
    {
      ...
      for (auto& [id, pSlot] : *pSlotMap) {
        ...
        slot[i] = pSlot;
        if ((ris = pSlotMap->begin()->second->transport.GetStatusChange(Context, 0, &state[i], 1)) != S_OK) {
    ...
    while (true) {
      Context.validate();
      ris = pSlotMap->begin()->second->transport.GetStatusChange(Context, 1000, state.data(), static_cast<DWORD>(dwSlotNum));
      if (ris != S_OK) {
...
void CSlot::InitSlotList(ISmartCardTransport &transport) {
  ...
  if ((ris = Context.transport.ListReaders(Context, &readers[0], &readersLen)) !=
      SCARD_S_SUCCESS)
    throw windows_error(ris);
...
void CSlot::GetTokenInfo(CK_TOKEN_INFO_PTR pInfo) {
  ...
  std::memcpy(reinterpret_cast<char*>(pInfo->manufacturerID), manifacturer.c_str(),
                     manifacturer.size());
  ...
  std::memcpy(reinterpret_cast<char*>(pInfo->utcTime), "1234567890123456", 16);  // OK
}
...
size_t CSlot::RWSessionCount() {
  ...
  for (auto& [handle, session] : CSession::g_mSessions) {
    if (session->pSlot.get() == this &&
        (session->flags & CKF_RW_SESSION) != 0)
      dwRWSessCount++;
  }
...
CK_OBJECT_HANDLE CSlot::GetIDFromObject(
  ...
  if (pPair == ObjP11Map.end()) {
    ...
    CK_OBJECT_HANDLE hObject = static_cast<CK_OBJECT_HANDLE>(reinterpret_cast<uintptr_t>(&pObject));  // GetNewObjectID();
...
ByteDynArray CSlot::GetATR() {
  init_func
      ...
      DWORD atrLen = 40;
  char ATR[40];
  long ret = transport.GetAttrib(this->hCard, SCARD_ATTR_ATR_STRING, reinterpret_cast<uint8_t*>(ATR),
                            &atrLen);
