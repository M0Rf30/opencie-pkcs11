        ...
        try {
          ...
          if (attr == nullptr)
            ...
            if (attr->size() != pTemplate[j].ulValueLen) {
              bSkip = true;
            } else if ((*attr) != ByteArray(static_cast<BYTE *>(pTemplate[j].pValue),
...
ByteDynArray GetTemplateValue(CK_ATTRIBUTE_PTR pTemplate, CK_ULONG ulCount,
  ...
  init_func for (unsigned int i = 0; i < ulCount; i++) {
    if (pTemplate[i].type == type) {
      return ByteArray(static_cast<uint8_t *>(pTemplate[i].pValue), pTemplate[i].ulValueLen);
    }
  }
...
void CSession::DigestInit(CK_MECHANISM_PTR pMechanism) {
  ...
  switch (pMechanism->mechanism) {
    case CKM_SHA_1: {
      auto mech =
          std::make_unique<CDigestSHA>(shared_from_this());
      mech->DigestInit();
    ...
    case CKM_SHA256: {
      auto mech =
          std::make_unique<CDigestSHA256>(shared_from_this());
      mech->DigestInit();
    ...
    case CKM_MD5: {
      auto mech =
          std::make_unique<CDigestMD5>(shared_from_this());
      mech->DigestInit();
...
void CSession::VerifyInit(CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey) {
  ...
  switch (pMechanism->mechanism) {
    case CKM_SHA1_RSA_PKCS: {
      auto mech =
          std::make_unique<CRSAwithSHA1>(shared_from_this());
      mech->VerifyInit(hKey);
      pVerifyMechanism = std::move(mech);
      break;
    }
    case CKM_SHA256_RSA_PKCS: {
      auto mech = std::make_unique<CRSAwithSHA256>(
          shared_from_this());
      mech->VerifyInit(hKey);
      pVerifyMechanism = std::move(mech);
      break;
    ...
    case CKM_MD5_RSA_PKCS: {
      auto mech =
          std::make_unique<CRSAwithMD5>(shared_from_this());
      mech->VerifyInit(hKey);
      pVerifyMechanism = std::move(mech);
      break;
    }
    case CKM_RSA_PKCS: {
      auto mech =
          std::make_unique<CRSA_PKCS1>(shared_from_this());
      mech->VerifyInit(hKey);
      pVerifyMechanism = std::move(mech);
      break;
    }
...
void CSession::GetOperationState(ByteArray &OperationState) {
  ...
  if (OperationState.isNull()) {
    OperationState = ByteArray(static_cast<uint8_t*>(nullptr), newOperationState.size());
    return;
  }
