...
/**
 * @file session.cpp
 * @brief PKCS#11 session operations implementation (sign, verify, digest, find
 * objects)
 */
          ...
          if (attr == nullptr)
            ...
            if (attr->size() != pTemplate[j].ulValueLen) {
              bSkip = true;
                       ...
                       ByteArray(static_cast<BYTE *>(pTemplate[j].pValue),
...
void CSession::DigestInit(CK_MECHANISM_PTR pMechanism) {
  ...
  switch (pMechanism->mechanism) {
    case CKM_SHA_1: {
      auto mech = std::make_unique<CDigestSHA>(shared_from_this());
      mech->DigestInit();
    ...
    case CKM_SHA256: {
      auto mech = std::make_unique<CDigestSHA256>(shared_from_this());
      mech->DigestInit();
    ...
    case CKM_MD5: {
      auto mech = std::make_unique<CDigestMD5>(shared_from_this());
      mech->DigestInit();
...
void CSession::VerifyInit(CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey) {
  ...
  switch (pMechanism->mechanism) {
    case CKM_SHA1_RSA_PKCS: {
      auto mech = std::make_unique<CRSAwithSHA1>(shared_from_this());
      mech->VerifyInit(hKey);
      pVerifyMechanism = std::move(mech);
      break;
    }
    case CKM_SHA256_RSA_PKCS: {
      auto mech = std::make_unique<CRSAwithSHA256>(shared_from_this());
      mech->VerifyInit(hKey);
      pVerifyMechanism = std::move(mech);
      break;
    }
    case CKM_MD5_RSA_PKCS: {
      auto mech = std::make_unique<CRSAwithMD5>(shared_from_this());
      mech->VerifyInit(hKey);
      pVerifyMechanism = std::move(mech);
      break;
    }
    case CKM_RSA_PKCS: {
      auto mech = std::make_unique<CRSA_PKCS1>(shared_from_this());
      mech->VerifyInit(hKey);
      pVerifyMechanism = std::move(mech);
      break;
    }
...
void CSession::SignFinal(ByteArray &Signature) {
  ...
  if (Signature.isNull()) {
    ...
    Signature = ByteArray(static_cast<uint8_t *>(nullptr), ulSignLength);
    mech.release();
    return;
  }
...
void CSession::SignRecover(ByteArray &Data, ByteArray &Signature) {
  ...
  if (Signature.isNull()) {
    ...
    Signature = ByteArray(static_cast<uint8_t *>(nullptr), ulSignRecoverLength);
    mech.release();
    return;
  }
...
void CSession::SetOperationState(ByteArray &OperationState) {
  ...
  if (!SignOperationState.isNull()) {
    ...
    CK_MECHANISM mech = {ByteArrayToVar(Algo, CK_MECHANISM_TYPE), nullptr, 0};
    ...
    std::shared_ptr<CP11Object> pKey = pSlot->FindP11Object(
        CKO_PRIVATE_KEY, CKA_ID, Key.data(), static_cast<int>(Key.size()));
  ...
  if (!VerifyOperationState.isNull()) {
    ...
    std::shared_ptr<CP11Object> pKey = pSlot->FindP11Object(
        CKO_PUBLIC_KEY, CKA_ID, Key.data(), static_cast<int>(Key.size()));
    ER_ASSERT(pKey != nullptr, ERR_CANT_GET_OBJECT)
    CK_OBJECT_HANDLE hKey = pSlot->GetIDFromObject(pKey);
...
void CSession::GetOperationState(ByteArray &OperationState) {
  ...
  if (pSignMechanism) {
    CTLVCreate SignTlv;
    ByteArray ba3(reinterpret_cast<BYTE *>(&pSignMechanism->mtType),
  ...
  if (pVerifyMechanism) {
    ...
    ByteArray ba(reinterpret_cast<BYTE *>(&pVerifyMechanism->mtType),
  ...
  if (pDigestMechanism) {
    ...
    ByteArray ba(reinterpret_cast<BYTE *>(&pDigestMechanism->mtType),
  ...
  if (OperationState.isNull()) {
    OperationState =
        ByteArray(static_cast<uint8_t *>(nullptr), newOperationState.size());
    return;
  }
