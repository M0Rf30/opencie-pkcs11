...
CK_RV CK_ENTRY cie_change_pin(const char* szCurrentPIN, const char* szNewPIN,
  ...
  if (szNewPIN == nullptr || strnlen(szNewPIN, 9) != 8) return CKR_PIN_LEN_RANGE;
  try {
    DWORD len = 0;
    ...
    for (; curreader[0] != 0; curreader += strnlen(curreader, len) + 1) {
      ...
      ByteArray oldPINBa(reinterpret_cast<const uint8_t*>(szCurrentPIN), strlen(szCurrentPIN));
      ...
      ByteArray newPINBa(reinterpret_cast<const uint8_t*>(szNewPIN), strlen(szNewPIN));
