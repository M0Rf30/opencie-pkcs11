...
extern "C" CK_RV CK_ENTRY cie_verify(const char* inFilePath,
  ...
  try {
    ...
    // If signers were found and no explicit error code is set, treat as
    // success even if the underlying pipeline returned a non-zero secondary
    // status (e.g. -2 from inner-document checks after all signatures were
    // verified).
    ...
    if (!hasSigners && (ret != 0 || verifyResult.nErrorCode != 0)) {
      ...
      LOG_ERROR("Verification error: %ld", err);
    ...
    LOG_ERROR("cie_verify unexpected exception: %s", e.what());
    return static_cast<CK_RV>(CIE_SIGN_ERROR_UNEXPECTED);
  } catch (...) {
    LOG_ERROR("cie_verify unexpected exception");
    return static_cast<CK_RV>(CIE_SIGN_ERROR_UNEXPECTED);
  }
}
...
extern "C" CK_RV CK_ENTRY cie_get_sign_count(void) {
  try {
    ...
    LOG_ERROR("cie_get_sign_count unexpected exception: %s", e.what());
    return static_cast<CK_RV>(CIE_SIGN_ERROR_UNEXPECTED);
  } catch (...) {
    LOG_ERROR("cie_get_sign_count unexpected exception");
    return static_cast<CK_RV>(CIE_SIGN_ERROR_UNEXPECTED);
  }
}
...
extern "C" CK_RV CK_ENTRY cie_get_verify_info(int index,
  ...
  try {
    ...
    snprintf(vInfos->name, OPENCIE_MAX_LEN * 2, "%s",
             tmpSignerInfo.szGIVENNAME);
    snprintf(vInfos->surname, OPENCIE_MAX_LEN * 2, "%s",
             tmpSignerInfo.szSURNAME);
    ...
    LOG_ERROR("cie_get_verify_info unexpected exception: %s", e.what());
    return static_cast<CK_RV>(CIE_SIGN_ERROR_UNEXPECTED);
  } catch (...) {
    LOG_ERROR("cie_get_verify_info unexpected exception");
    return static_cast<CK_RV>(CIE_SIGN_ERROR_UNEXPECTED);
  }
}
...
extern "C" CK_RV CK_ENTRY cie_extract_p7m(const char* inFilePath,
  ...
  try {
    ...
    LOG_ERROR("cie_extract_p7m unexpected exception: %s", e.what());
    return static_cast<CK_RV>(CIE_SIGN_ERROR_UNEXPECTED);
  } catch (...) {
    LOG_ERROR("cie_extract_p7m unexpected exception");
    return static_cast<CK_RV>(CIE_SIGN_ERROR_UNEXPECTED);
  }
}
