...
long CIEVerify::verify(const char* input_file, VERIFY_RESULT* verifyResult,
  ...
  try {
    ...
    ret = disigon_set(DISIGON_OPT_LOG_LEVEL,
                      reinterpret_cast<void*>(LOG_TYPE_DEBUG));
#endif

    ret = disigon_verify_set(ctx, DISIGON_OPT_INPUTFILE,
    ...
    ret = disigon_verify_set(ctx, DISIGON_OPT_INPUTFILE_TYPE,
                             reinterpret_cast<void*>(DISIGON_FILETYPE_AUTO));
    if (ret != 0) {
      throw ret;
    }
    ...
    // PARAMETRO 1 OK OCSP
    ret = disigon_verify_set(ctx, DISIGON_OPT_VERIFY_REVOCATION,
    ...
    if (proxy_address) {
      ret = disigon_verify_set(
          ctx, DISIGON_OPT_PROXY,
...
long CIEVerify::get_file_from_p7m(const char* input_file,
  ...
  try {
    ...
    ret =
        disigon_verify_set(ctx, DISIGON_OPT_OUTPUTFILE,
