...
GitHub Actions run five jobs on every push:

| Job | Runner | What it does |
|---|---|---|
| `linux` | Ubuntu | Native build, clang-tidy, uploads `.so` |
| `windows` | Ubuntu + MinGW | Cross-compile, uploads `.dll` |
| `portable` | Docker (Ubuntu base) | x86\_64 + aarch64 portable builds, symbol count check, glibc version check |
| `macos` | macOS | Native build, symbol verification, uploads `.dylib` |
| `android` | Ubuntu + NDK r27c | arm64 cross-compile via vcpkg, uploads `.so` |
