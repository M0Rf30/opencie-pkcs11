# Build container for distro-independent libcie-pkcs11.so
#
...
#
# Usage:
#   podman build -t libcie-builder -f Containerfile .
#   podman run --rm -v $(pwd)/output:/output libcie-builder
#
# For aarch64 cross-compilation:
#   podman run --rm -v $(pwd)/output:/output libcie-builder aarch64
...
echo "=== Building libcie-pkcs11.so (portable, ${ARCH}) ==="
...
readelf -d "${BUILDDIR}/libcie-pkcs11.so" | grep NEEDED || true
echo ""
echo "--- Exported symbols ---"
nm -D "${BUILDDIR}/libcie-pkcs11.so" | grep -c ' T ' || true
echo ""
echo "--- glibc version requirement ---"
readelf -V "${BUILDDIR}/libcie-pkcs11.so" 2>/dev/null | grep -oP 'GLIBC_\S+' | sort -V | tail -1 || true
echo ""
echo "--- File info ---"
file "${BUILDDIR}/libcie-pkcs11.so"
ls -lh "${BUILDDIR}/libcie-pkcs11.so"

# Copy to output if mounted
if [ -d /output ]; then
    cp "${BUILDDIR}/libcie-pkcs11.so" "/output/libcie-pkcs11-${ARCH}.so"
    echo ""
    echo "Output copied to /output/libcie-pkcs11-${ARCH}.so"
fi
SCRIPT
