...
case "$ARCH" in
    ...
    aarch64)
        ...
        # Substitute the repo checkout path into the cross file so Meson sees
        # an absolute cmake_toolchain_file (Meson requirement).
        sed -i "s|@SOURCE_ROOT@|$(pwd)|g" toolchains/cross-aarch64.ini
        CROSS_ARGS="--cross-file toolchains/cross-aarch64.ini"
        ;;
    *)
