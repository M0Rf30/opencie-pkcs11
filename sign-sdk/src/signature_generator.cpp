...
CSignatureGeneratorBase::CSignatureGeneratorBase(CBaseSigner* pSigner)
    : m_pSigner(pSigner), m_nHashAlgo(CKM_SHA1_RSA_PKCS), m_pTSAClient(nullptr) {
  m_szAlias[0] = 0;
}
...
void CSignatureGeneratorBase::SetData(const UUCByteArray& data) {
  m_data.append(const_cast<BYTE*>(data.getContent()), data.getLength());
}
...
void CSignatureGenerator::SetPKCS7Data(const UUCByteArray& pkcs7data) {
  try {
    CSignedDocument sd(const_cast<BYTE*>(pkcs7data.getContent()), pkcs7data.getLength());
...
long CSignatureGenerator::Generate(UUCByteArray& pkcs7SignedData,
  ...
  // hash certificate
  ByteArray baCert(certval.getContent(), certval.getLength());
  ...
  m_signerInfoGenerator.setSigningCertificate(
      certval.getContent(), certval.getLength(), certHashBuf.data(), 32);
  ...
  switch (mech) {
    case CKM_SHA256_RSA_PKCS: {
      ...
      {
        ByteArray baData(m_data.getContent(), m_data.getLength());
      ...
      m_signerInfoGenerator.getSignedAttributes(signedAttributes, false,
                                                !bDetached);
      {
        ByteArray baSA(signedAttributes.getContent(), signedAttributes.getLength());
    ...
    case CKM_SHA1_RSA_PKCS: {
      ...
      EVP_DigestUpdate(sha1_ctx, const_cast<BYTE*>(m_data.getContent()),
                       m_data.getLength());
      ...
      memcpy(hashBuf.data(), hashaux.getContent(), hashlen);
      ...
      memcpy(hashBuf.data(), hashaux1.getContent(), hashlen);
    } break;
  }
  ...
  if (m_bRemote) {
    digest.append(hashBuf.data(), hashlen);
  } else {
  ...
  m_signerInfoGenerator.setSignature(const_cast<BYTE*>(signature.getContent()),
                                     signature.getLength());
  ...
  if (m_pTSAClient != nullptr) {
    ...
    {
      ByteArray baContent(content.getContent(), content.getLength());
      ByteDynArray h = CSHA256().Digest(baContent);
      memcpy(hashBuf.data(), h.data(), 32);
    }

    UUCByteArray hashaux(hashBuf.data(), hashlen);
