  ...
  try {
...

    {
      ...
      while ((nRead = fread(buffer, 1, BUFFERSIZE, f)) > 0) {
        m_data.append(ByteArray(buffer, nRead));
      }
      fclose(f);
    }
...
int PDFVerifier::VerifySignature(const PdfMemDocument *pDoc,
  ...
  if (signature->IsDictionary()) {
    ...
    std::string hexData =
        (ltPos != std::string::npos && gtPos != std::string::npos)
            ? signdData.substr(ltPos + 1, gtPos - ltPos - 1)
            : signdData;

    ByteDynArray baSignedData(hexData.c_str());
    CSignedDocument signedDocument(baSignedData.data(), baSignedData.size());
    ...
    if (subfilter == "/adbe.pkcs7.detached" ||
      ...
      ByteDynArray baContent;
      baContent.append(ByteArray(reinterpret_cast<BYTE *>(m_szDocBuffer), len));
      baContent.append(
          ByteArray((reinterpret_cast<BYTE *>(m_szDocBuffer) + start1), len1));
...
int PDFVerifier::GetSignature(size_t index, ByteDynArray &signedDocument,
...
int PDFVerifier::GetSignature(const PdfMemDocument *pDoc,
  ...
  if (signature->IsDictionary()) {
    ...
    std::string hexData2 =
        (ltPos2 != std::string::npos && gtPos2 != std::string::npos)
