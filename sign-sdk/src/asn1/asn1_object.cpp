...
bool CASN1Object::operator==(const CASN1Object& obj) const {
  ...
  int r = memcmp(static_cast<const void*>(val1), static_cast<const void*>(val2), getLength());
  return r == 0;
}
...
void CASN1Object::toByteArray(UUCByteArray& byteArray) const {
  ...
  if (nLen < 0x80) {
    ...
    memcpy((serialized.data() + 2), getValue()->getContent(), nLen);
    ...
    memcpy((serialized.data() + 2 + (nLenNeeded)), getValue()->getContent(), nLen);
  }

  byteArray.append(serialized.data(), nTLVLen);
...
int CASN1Object::parseLen(UUCBufferedReader& reader, BYTE* pbtTag,
  ...
  if (btLenRead == 0x80) {  // indefinite-length
    ...
    for (BYTE i = 0; i < btLenRead; i++) {
      nLen += static_cast<UINT>(btHexLen[btLenRead - i - 1] * pow(static_cast<double>(256), i));
    }
  ...
  if (pValue) {
    ...
    pValue->append(pbtVal.data(), nLen);
  }
  return nLen;
}
