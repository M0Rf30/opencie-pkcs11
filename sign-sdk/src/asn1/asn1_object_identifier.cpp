...
CASN1ObjectIdentifier::CASN1ObjectIdentifier(const char* strObjId)
  ...
  while (std::getline(iss, token, '.')) {
    ...
    if (nVal == 0) {
      ...
      int i = static_cast<int>(ceil((log(static_cast<double>(abs(nVal))) / log(static_cast<double>(2))) / 7));
      while (nVal != 0) {
        nAux = static_cast<int>(floor(nVal / pow(static_cast<float>(128), i - 1)));
        nVal = nVal - static_cast<int>(pow(static_cast<float>(128), i - 1) * nAux);
        if (nVal != 0) nAux |= 0x80;
        out[nIndex++] = nAux;
        i--;
      }
...
void CASN1ObjectIdentifier::ToOidString(UUCByteArray& objId) {
  ...
  for (int i = 0; i != len; i++) {
    ...
    if ((b & 0x80) == 0) {  // end of number reached
      ...
      objId.append(reinterpret_cast<BYTE*>(szValue), strlen(szValue));
      value = 0;
    }
  }

  objId.append(static_cast<BYTE>('\0'));
}
