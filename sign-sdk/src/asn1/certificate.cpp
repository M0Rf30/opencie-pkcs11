...
CCertificate* CCertificate::createCertificate(UUCByteArray& contentArray) {
  ...
  if (content[0] != 0x30) {  // base64
    std::string encoded(reinterpret_cast<const char*>(content), static_cast<size_t>(len));
    ...
    UUCBufferedReader reader(decoded.data(), decoded.size());
    ...
    UUCBufferedReader reader(const_cast<BYTE*>(content), len);
...
bool CCertificate::isValid(const char* szDateTime) {
  ...
  if (expiration.getValue()->getLength() > 13) {
    btExpiration = const_cast<BYTE*>(expiration.getValue()->getContent()) +
  ...
  if (from.getValue()->getLength() > 13) {
    btFrom = const_cast<BYTE*>(from.getValue()->getContent()) +
...
int CCertificate::verifyStatus(const char* szTime,
  ...
  try {
    ...
    UUCByteArray* pbaVal = const_cast<UUCByteArray*>(val.getValue());
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (oid.equals(accessDescription.elementAt(0))) {
        ...
        UUCByteArray* pValue = const_cast<UUCByteArray*>(accessLocation.getValue());
        pValue->append(static_cast<BYTE>('\0'));

        LOG_DBG((0, "CCertificate::verifyStatus", "OCSP Url: %s",
                 reinterpret_cast<const char*>(pValue->getContent())));
        ...
        long nRet = HTTPRequest(baOcspRequest, reinterpret_cast<const char*>(pValue->getContent()),
              ...
              // verifica CRLReason
              {
                ...
                try {
                  ...
                  if (revocationTime.getValue()->getLength() > 13) {
                    btRevocationTime =
                        const_cast<BYTE*>(revocationTime.getValue()->getContent()) +
  ...
  try {
    ...
    if (size > 0) {
      for (int i = 0; i < size; i++) {
        ...
        UUCByteArray* pValue = const_cast<UUCByteArray*>(name3.getValue());

        pValue->append(static_cast<BYTE>('\0'));

        const char* szcrlurl = reinterpret_cast<const char*>(pValue->getContent());
...
bool CCertificate::verifySignature(CCertificate& cert) {
  ...
  x509 = d2i_X509(nullptr, &content, baCert.getLength());
  ...
  UUCByteArray encDigest(*const_cast<UUCByteArray*>(encryptedDigest.getValue()));
  encDigest.remove(0);
  ...
  const int encrypted_len = static_cast<int>(encDigest.getLength());
  ...
  if (len) {
    try {
      ...
      UUCByteArray* pDigestValue = const_cast<UUCByteArray*>(digest.getValue());
      ...
      buff = const_cast<BYTE*>(content.getContent());
...
static size_t WriteCallback(void* contents, size_t size, size_t nmemb,
                            void* userp) {
  UUCByteArray* pResponse = static_cast<UUCByteArray*>(userp);
  size_t realsize = size * nmemb;
  pResponse->append(static_cast<BYTE*>(contents), realsize);
