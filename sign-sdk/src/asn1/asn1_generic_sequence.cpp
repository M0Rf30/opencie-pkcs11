...
CASN1GenericSequence::CASN1GenericSequence(BYTE btTag)
  ...
  m_pnOffsets =
      static_cast<unsigned int*>(calloc(m_nOffsetsMax + 2, sizeof(m_pnOffsets[0])));
  setTag(btTag);
}
...
CASN1GenericSequence::CASN1GenericSequence(const CASN1Object& obj)
    : CASN1Object(obj), m_pnOffsets(nullptr), m_nOffsetsMax(MAXSIZE), m_nSize(0) {
  m_pnOffsets =
      static_cast<unsigned int*>(calloc(m_nOffsetsMax + 2, sizeof(m_pnOffsets[0])));
  m_nSize = makeOffset();
}
...
int CASN1GenericSequence::makeOffset() {
  ...
  while (offset < len) {
    if (i == m_nOffsetsMax) {
      m_nOffsetsMax += 1000;
      m_pnOffsets = static_cast<unsigned int*>(realloc(
