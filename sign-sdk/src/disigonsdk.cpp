...
static long readFileIntoByteArray(const char* path, UUCByteArray& data) {
  BYTE buffer[BUFFERSIZE];
  int nRead = 0;
  FILE* f = fopen(path, "rb");
  if (!f) return DISIGON_ERROR_FILE_NOT_FOUND;
...
static long writeByteArrayToFile(const char* path, const UUCByteArray& data) {
  FILE* f = fopen(path, "w+b");
  if (!f) return DISIGON_ERROR_FILE_NOT_FOUND;
  fwrite(data.getContent(), 1, data.getLength(), f);
  fclose(f);
  return 0;
}
...
DISIGON_CTX disigon_sign_init(void) {
  ...
  snprintf(pContext->szPdfSubfilter, MAX_PATH, "%s", DISIGON_PDF_SUBFILTER_PKCS_DETACHED);
...
long disigon_sign_set(DISIGON_CTX ctx, int option, void* value) {
  ...
  DISIGON_SIGN_CONTEXT* pContext = static_cast<DISIGON_SIGN_CONTEXT*>(ctx);
  ...
  switch (option) {
    ...
    case DISIGON_OPT_INPUTFILE:
      ...
      snprintf(pContext->szInputFile, MAX_PATH, "%s", static_cast<char*>(value));
      break;

    case DISIGON_OPT_OUTPUTFILE:
      LOG_MSG((0, "disigon_sign_set", "Context: %p, Option: %s, Value: %s", ctx,
               "DISIGON_OPT_OUTPUTFILE", (char*)value));
      snprintf(pContext->szOutputFile, MAX_PATH, "%s", static_cast<char*>(value));
      break;
    ...
    case DISIGON_OPT_TSA_USERNAME:
      ...
      snprintf(pContext->szTSAUsername, MAX_PATH, "%s", static_cast<char*>(value));
    ...
    case DISIGON_OPT_TSA_PASSWORD:
      ...
      snprintf(pContext->szTSAPassword, MAX_PATH, "%s", static_cast<char*>(value));
      break;

    case DISIGON_OPT_PDF_SUBFILTER:
      LOG_MSG((0, "disigon_sign_set", "Context: %p, Option: %s, Value: %s", ctx,
               "DISIGON_OPT_PDF_SUBFILTER", (char*)value));
      snprintf(pContext->szPdfSubfilter, MAX_PATH, "%s", static_cast<char*>(value));
      break;

    case DISIGON_OPT_PDF_LOCATION:
      LOG_MSG((0, "disigon_sign_set", "Context: %p, Option: %s, Value: %s", ctx,
               "DISIGON_OPT_LOCATION", (char*)value));
      snprintf(pContext->szPdfLocation, MAX_PATH, "%s", static_cast<char*>(value));
      break;
    ...
    case DISIGON_OPT_PDF_REASON:
      ...
      snprintf(pContext->szPdfReason, MAX_PATH, "%s", static_cast<char*>(value));
      break;
    ...
    case DISIGON_OPT_PDF_IMAGEPATH:
      ...
      snprintf(pContext->szPdfImagePath, MAX_PATH, "%s", static_cast<char*>(value));
      break;
    ...
    case DISIGON_OPT_PDF_DESCRIPTION:
      ...
      snprintf(pContext->szPdfDescription, MAX_PATH, "%s", static_cast<char*>(value));
      break;

    case DISIGON_OPT_PDF_LOCATION_LABEL:
      LOG_MSG((0, "disigon_sign_set", "Context: %p, Option: %s, Value: %s", ctx,
               "DISIGON_OPT_PDF_LOCATION_LABEL", (char*)value));
      snprintf(pContext->szPdfLocationLabel, MAX_PATH, "%s", static_cast<char*>(value));
      break;

    case DISIGON_OPT_PDF_NAME_LABEL:
      LOG_MSG((0, "disigon_sign_set", "Context: %p, Option: %s, Value: %s", ctx,
               "DISIGON_OPT_PDF_NAME_LABEL", (char*)value));
      snprintf(pContext->szPdfNameLabel, MAX_PATH, "%s", static_cast<char*>(value));
      break;

    case DISIGON_OPT_PDF_REASON_LABEL:
      LOG_MSG((0, "disigon_sign_set", "Context: %p, Option: %s, Value: %s", ctx,
               "DISIGON_OPT_PDF_REASON_LABEL", (char*)value));
      snprintf(pContext->szPdfReasonLabel, MAX_PATH, "%s", static_cast<char*>(value));
      break;
...
long disigon_sign_sign(DISIGON_CTX ctx) {
  ...
  if (pContext->szOutputFile[0] == 0) {
    snprintf(pContext->szOutputFile, sizeof(pContext->szOutputFile), "%.1019s.p7m", pContext->szInputFile);
  }
...
long disigon_verify_set(DISIGON_CTX ctx, int option, void* value) {
  ...
  DISIGON_VERIFY_CONTEXT* pContext = static_cast<DISIGON_VERIFY_CONTEXT*>(ctx);
  ...
  switch (option) {
    ...
    case DISIGON_OPT_INPUTFILE_PLAINTEXT:
      snprintf(pContext->szInputPlainTextFile, MAX_PATH, "%s", static_cast<char*>(value));
      break;
...
long disigon_verify_verify(DISIGON_CTX ctx, VERIFY_RESULT* pVerifyResult) {
  ...
  switch (nFileType) {
    case DISIGON_FILETYPE_P7M:
      ...
      snprintf(pVerifyResult->szPlainTextFile, MAX_PATH, "%s", pContext->szInputFile);
    ...
    case DISIGON_FILETYPE_XML:
      ...
      for (int i = 0; i < pVerifyResult->verifyInfo.pSignerInfos->nCount; i++) {
        if (pVerifyResult->verifyInfo.pSignerInfos->pSignerInfo[i].pTimeStamp) {
          if ((static_cast<TS_INFO*>(pVerifyResult->verifyInfo.pSignerInfos->pSignerInfo[i]
                   .pTimeStamp))
...
long verify_xml(DISIGON_VERIFY_CONTEXT* pContext, VERIFY_INFO* pVerifyInfo) {
  ...
  for (int i = 0; i < signatureCount; i++) {
    ...
    snprintf(pSI->szDigestAlgorithm, MAX_LEN, "%s", (char*)oid.getContent());
    ...
    snprintf(pSI->szDN, MAX_LEN * 2, "%s", (char*)subject.getContent());
    snprintf(pSI->szSN, MAX_LEN * 2, "%s",
           (const_cast<UUCByteArray*>(pCert->getSerialNumber().getValue()))->toHexString());
    ...
    for (int j = 0; j < count; j++) {
      ...
      const char* hexval = (const_cast<UUCByteArray*>(value.getValue()))->toHexString();
      char* szAux = new char[strlen(szoid) + strlen(hexval) + 5];
      snprintf(szAux, strlen(szoid) + strlen(hexval) + 5, "%s:%s", szoid, hexval);
    ...
    snprintf(pSI->szCADN, MAX_LEN * 2, "%s", (char*)issuer.getContent());
...
long verify_m7m(DISIGON_VERIFY_CONTEXT* pContext, VERIFY_INFO* pVerifyInfo) {
  ...
  if (sd.isDetached()) {
    if (pContext->szInputPlainTextFile[0] != '\0') {
      ...
      if (readFileIntoByteArray(pContext->szInputPlainTextFile, fileData) != 0) {
        LOG_ERR((0, "<-- verify_m7m",
                 "Context: %p, Error: DISIGON_ERROR_FILE_NOT_FOUND, file: %s",
                 pContext, pContext->szInputPlainTextFile));
        return DISIGON_ERROR_FILE_NOT_FOUND;
...
SIGNER_INFO* verify_countersignature(DISIGON_VERIFY_CONTEXT* pContext,
  ...
  if (counterSignatureCount > 0) {
    ...
    for (int i = 0; i < counterSignatureCount; i++) {
      ...
      SIGNER_INFO* pSI = &((static_cast<SIGNER_INFO*>(pSignerInfo->pCounterSignatures))[i]);
      pSI->pTimeStamp = nullptr;
      pSI->pCounterSignatures = nullptr;
      pSI->nCounterSignatureCount = 0;
      pSI->pRevocationInfo = nullptr;
      ...
      snprintf(pSI->szSN, MAX_LEN * 2, "%s",
             (const_cast<UUCByteArray*>(cert.getSerialNumber().getValue()))->toHexString());
      ...
      snprintf(pSI->szCertificateV2, MAX_LEN, "%s", szSHA256OID);  // default value
      ...
      if (si.hasTimeStampToken()) {
        ...
        snprintf(pTSInfo->signerInfo.szDN, MAX_LEN * 2, "%s", (char*)subject.getContent());
        snprintf(pTSInfo->signerInfo.szCADN, MAX_LEN * 2, "%s", (char*)issuer.getContent());

        CTSTInfo tstInfo(tst.getTSTInfo());

        snprintf(
            pTSInfo->signerInfo.szSN, MAX_LEN * 2, "%s",
            (const_cast<UUCByteArray*>(cert.getSerialNumber().getValue()))->toHexString());
        snprintf(pTSInfo->szTimeStampSerial, MAX_LEN, "%s",
               (const_cast<UUCByteArray*>(tstInfo.getSerialNumber().getValue()))
                   ->toHexString());
        ...
        UUCByteArray* certificate = const_cast<UUCByteArray*>(tsacert.getValue());
        ...
        snprintf(pTSInfo->szTimeStampImprintAlgorithm, MAX_LEN, "%s", (char*)oid1.getContent());

        // imprint b64
        CASN1OctetString mimprint = messageImprint.elementAt(1);
        const UUCByteArray* val = mimprint.getValue();

        ByteArray ba(const_cast<BYTE*>(val->getContent()), val->getLength());
        std::string b64Encoded;
        CBase64().Encode(ba, b64Encoded);

        snprintf(pTSInfo->szTimeStampMessageImprint, MAX_LEN, "%s", b64Encoded.c_str());
        ...
        UUCByteArray oid;
        digestOID.ToOidString(oid);
        snprintf(pTSInfo->signerInfo.szDigestAlgorithm, MAX_LEN, "%s", (char*)oid.getContent());
  ...
  for (int i = 0; i < sigCount; i++) {
    ...
    if (si.hasTimeStampToken()) {
      ...
      snprintf(pTSInfo->signerInfo.szSN, MAX_LEN * 2, "%s",
             (const_cast<UUCByteArray*>(cert.getSerialNumber().getValue()))->toHexString());
      snprintf(
          pTSInfo->szTimeStampSerial, MAX_LEN, "%s",
          (const_cast<UUCByteArray*>(tstInfo.getSerialNumber().getValue()))->toHexString());
...
long sign_xml(DISIGON_SIGN_CONTEXT* pContext, UUCByteArray& data) {
  ...
  if (pContext->szOutputFile[0] == 0) {
    if (pContext->bDetached)
      snprintf(pContext->szOutputFile, sizeof(pContext->szOutputFile), "signed_%.1012s.xml", pContext->szInputFile);
    else
      snprintf(pContext->szOutputFile, sizeof(pContext->szOutputFile), "%.1019s.xml", pContext->szInputFile);
  }
...
long sign_pdf(DISIGON_SIGN_CONTEXT* pContext, UUCByteArray& data) {
  ...
  if (pContext->szPdfImagePath[0] != 0 || pContext->szPdfDescription[0] != 0 ||
    ...
    if (!pContext->szPdfReason[0]) {
      ...
      if (r == 0) {
        std::string giveName = pCertificate->getSubject().getField(OID_GIVEN_NAME);
  ...
  if (pContext->szOutputFile[0] == 0) {
    snprintf(pContext->szOutputFile, sizeof(pContext->szOutputFile), "%.1019s.pdf", pContext->szInputFile);
  }
...
long verify_tsd(DISIGON_VERIFY_CONTEXT* pContext, VERIFY_INFO* pVerifyInfo) {
  ...
  if (data.getContent()[0] != 0x30) {
    // base64
    std::string encoded(reinterpret_cast<const char*>(data.getContent()),
    ...
    if (dash != std::string::npos) {
      ...
      if (start != std::string::npos)
        encoded = encoded.substr(start + 1,
                                 end != std::string::npos ? end - (start + 1)
                                                          : std::string::npos);
    }
    ...
    pbContent = const_cast<BYTE*>(data.getContent());
    len = data.getLength();
  }
...
long verifyTST(CTimeStampToken& tst, TS_INFO* pTSInfo, BOOL bVerifyCRL) {
  ...
  snprintf(pTSInfo->signerInfo.szSN, MAX_LEN * 2, "%s",
         (const_cast<UUCByteArray*>(tsacert.getSerialNumber().getValue()))->toHexString());
  snprintf(pTSInfo->szTimeStampSerial, MAX_LEN, "%s",
         (const_cast<UUCByteArray*>(tstInfo.getSerialNumber().getValue()))->toHexString());
...
void CIEPdfSigner::ComputeSignature(charbuff& buffer, bool dryrun) {
  if (dryrun) {
    ...
    long nRes;
    UUCByteArray toSign(reinterpret_cast<BYTE*>(m_buffer.data()), m_buffer.size());
