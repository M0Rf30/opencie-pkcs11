...
void BigUnsigned::setBit(Index bi, bool newBit) {
  Index blockI = bi / N;
  Blk block = getBlock(blockI), mask = static_cast<Blk>(1) << (bi % N);
...
void BigUnsigned::multiply(const BigUnsigned &a, const BigUnsigned &b) {
  ...
  for (i = 0; i < a.len; i++) {
    ...
    for (i2 = 0; i2 < N; i2++) {
      if ((a.blk[i] & (static_cast<Blk>(1) << i2)) == 0) continue;
      /*
...
void BigUnsigned::divideWithRemainder(const BigUnsigned &b, BigUnsigned &q) {
  ...
  std::vector<Blk> subtractBuf(len);
  ...
  zapLeadingZeros();
  // subtractBuf automatically deallocated by std::vector destructor.
}
