  ...
  try {
    ...
    switch (algo) {
      case CKM_SHA256_RSA_PKCS:
        make_digest_info(NID_sha256, const_cast<unsigned char*>(data.getContent()),
                       static_cast<size_t>(data.getLength()), digestinfo,
                       &digestinfolen);

        break;

      case CKM_SHA1_RSA_PKCS:
        make_digest_info(NID_sha1, const_cast<unsigned char*>(data.getContent()),
                       static_cast<size_t>(data.getLength()), digestinfo,
                       &digestinfolen);
        break;
    ...
    signature.append(baSignature.data(), static_cast<int>(baSignature.size()));
  } catch (const scard_error& err) {
    return err.sw;
  } catch (...) {
    return -1;
