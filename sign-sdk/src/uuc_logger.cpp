...
logFunc pfnCrashliticsLog = nullptr;
...
void UUCLogger::setLogFile(const char* szLogFileName) {
  snprintf(m_szLogFileName, sizeof(m_szLogFileName), "%s", szLogFileName);
}
...
void UUCLogger::log(const unsigned int nType, const char* szMsg,
  ...
  char szLogMsg[5000];
  snprintf(szLogMsg, sizeof(szLogMsg), "[%s], %d, %X, %s, %s\n", szTime, nType, nID, szModuleName,
          szMsg);
  printf("%s", szLogMsg);
  if (pfnCrashliticsLog != nullptr) pfnCrashliticsLog(szLogMsg);
}
