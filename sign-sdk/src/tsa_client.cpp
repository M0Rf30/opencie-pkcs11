...
void CTSAClient::SetTSAUrl(const char* szUrl) { snprintf(m_szTSAUrl, sizeof(m_szTSAUrl), "%s", szUrl); }
...
long CTSAClient::GetTimeStampToken(UUCByteArray& digest, const char* szPolicyID,
  ...
  try {
    CTimeStampResponse tsResponse(const_cast<BYTE*>(tsdata.getContent()),
                                  static_cast<int>(tsdata.getLength()));
...
static size_t WriteCallback(void* contents, size_t size, size_t nmemb,
                            void* userp) {
  UUCByteArray* pData = static_cast<UUCByteArray*>(userp);

  size_t realsize = size * nmemb;
  pData->append(static_cast<BYTE*>(contents), realsize);
