...
long CXAdESGenerator::Generate(UUCByteArray& xadesData, BOOL bDetached,
  ...
  pDoc = xmlParseMemory(reinterpret_cast<const char*>(m_data.getContent()), m_data.getLength());

  if (pDoc == nullptr) return DISIGON_ERROR_INVALID_FILE;
  ...
  UUCByteArray data(reinterpret_cast<const BYTE*>(strSigInfoCanonicalForm.c_str()),
                    strSigInfoCanonicalForm.length());

  ByteDynArray hashauxDyn;
  CBase64().Decode(strSignedInfoHashB64.c_str(), hashauxDyn);
  UUCByteArray hashaux(hashauxDyn.data(), static_cast<unsigned long>(hashauxDyn.size()));
  ...
  {
    ByteArray baSig(signature.getContent(), signature.getLength());
  ...
  {
    ByteArray baCertArg(baCert.getContent(), baCert.getLength());
...
void CXAdESGenerator::CanonicalizeAndHashBase64(xmlDocPtr pDoc,
  ...
  int docLen =
      xmlC14NDocDumpMemory(pDoc, nullptr, XML_C14N_1_0, nullptr, 0, &pCanonicalDoc);
  ...
  if (m_bXAdES) {
    ByteArray baCan(reinterpret_cast<const uint8_t*>(pCanonicalDoc), static_cast<size_t>(docLen));
    ByteDynArray sha256res = CSHA256().Digest(baCan);
    hashaux.append(sha256res.data(), 32);
  } else {
    // calcola l'hash SHA1
  ...
  strDocHashB64.clear();
  ByteArray baHash(hashaux.getContent(), hashaux.getLength());
  CBase64().Encode(baHash, strDocHashB64);
}

xmlDocPtr CXAdESGenerator::CreateSignedInfo(
    xmlDocPtr pDocument, std::string& strQualifyingPropertiesB64Hash, bool bDetached,
    char* szFileName) {
  // XML doc
  ...
  xmlNodePtr pSignedInfo =
      xmlNewChild(pSignatureNode, nullptr, reinterpret_cast<const xmlChar*>("ds:SignedInfo"), nullptr);
  ...
  xmlNodePtr pN1 = xmlNewChild(
      pSignedInfo, nullptr, reinterpret_cast<const xmlChar*>("ds:CanonicalizationMethod"), nullptr);
  xmlSetProp(pN1, reinterpret_cast<const xmlChar*>("Algorithm"),
             reinterpret_cast<const xmlChar*>("http://www.w3.org/TR/2001/REC-xml-c14n-20010315"));

  // <SignatureMethod>
  pN1 = xmlNewChild(pSignedInfo, nullptr, reinterpret_cast<const xmlChar*>("ds:SignatureMethod"),
                    nullptr);
  if (m_bXAdES) {
    xmlSetProp(
        pN1, reinterpret_cast<const xmlChar*>("Algorithm"),
        reinterpret_cast<const xmlChar*>("http://www.w3.org/2001/04/xmldsig-more#rsa-sha256"));
  } else {
    xmlSetProp(pN1, reinterpret_cast<const xmlChar*>("Algorithm"),
               reinterpret_cast<const xmlChar*>("http://www.w3.org/2000/09/xmldsig#rsa-sha1"));
  }

  // <Reference>
  pN1 = xmlNewChild(pSignedInfo, nullptr, reinterpret_cast<const xmlChar*>("ds:Reference"), nullptr);
  if (bDetached) {
    xmlSetProp(pN1, reinterpret_cast<const xmlChar*>("URI"), reinterpret_cast<const xmlChar*>(szFileName));
  } else {
    xmlSetProp(pN1, reinterpret_cast<const xmlChar*>("URI"), reinterpret_cast<const xmlChar*>(""));
  }

  if (!bDetached) {
    // Transformation
    xmlNodePtr pN2 =
        xmlNewChild(pN1, nullptr, reinterpret_cast<const xmlChar*>("ds:Transforms"), nullptr);
    xmlNodePtr pN3 =
        xmlNewChild(pN2, nullptr, reinterpret_cast<const xmlChar*>("ds:Transform"), nullptr);

    // filter xpath
    xmlSetProp(pN3, reinterpret_cast<const xmlChar*>("Algorithm"),
               reinterpret_cast<const xmlChar*>("http://www.w3.org/2002/06/xmldsig-filter2"));

    xmlNodePtr pN31 = xmlNewChild(pN3, nullptr, reinterpret_cast<const xmlChar*>("dsig-xpath:XPath"),
                                  reinterpret_cast<const xmlChar*>("/descendant::ds:Signature"));
    xmlSetProp(pN31, reinterpret_cast<const xmlChar*>("Filter"), reinterpret_cast<const xmlChar*>("subtract"));

    // c14N
    xmlNodePtr pN4 =
        xmlNewChild(pN2, nullptr, reinterpret_cast<const xmlChar*>("ds:Transform"), nullptr);
    xmlSetProp(
        pN4, reinterpret_cast<const xmlChar*>("Algorithm"),
        reinterpret_cast<const xmlChar*>("http://www.w3.org/TR/2001/REC-xml-c14n-20010315"));
  }

  // documents digest
  xmlNodePtr pN2 =
      xmlNewChild(pN1, nullptr, reinterpret_cast<const xmlChar*>("ds:DigestMethod"), nullptr);
  xmlSetProp(pN2, reinterpret_cast<const xmlChar*>("Algorithm"),
             reinterpret_cast<const xmlChar*>("http://www.w3.org/2001/04/xmlenc#sha256"));
  ...
  pN2 = xmlNewChild(pN1, nullptr, reinterpret_cast<const xmlChar*>("ds:DigestValue"),
  ...
  if (!strQualifyingPropertiesB64Hash.empty()) {
    ...
    xmlSetProp(pN1, reinterpret_cast<const xmlChar*>("Type"),
               reinterpret_cast<const xmlChar*>("http://uri.etsi.org/01903#SignedProperties"));
    ...
    // c14N
    xmlNodePtr pN04 =
        xmlNewChild(pN02, nullptr, reinterpret_cast<const xmlChar*>("ds:Transform"), nullptr);
    xmlSetProp(
        pN04, reinterpret_cast<const xmlChar*>("Algorithm"),
        reinterpret_cast<const xmlChar*>("http://www.w3.org/TR/2001/REC-xml-c14n-20010315"));

    // documents digest
    pN2 = xmlNewChild(pN1, nullptr, reinterpret_cast<const xmlChar*>("ds:DigestMethod"), nullptr);
    xmlSetProp(pN2, reinterpret_cast<const xmlChar*>("Algorithm"),
               reinterpret_cast<const xmlChar*>("http://www.w3.org/2001/04/xmlenc#sha256"));
    pN2 = xmlNewChild(pN1, nullptr, reinterpret_cast<const xmlChar*>("ds:DigestValue"),
                      reinterpret_cast<const xmlChar*>(strQualifyingPropertiesB64Hash.c_str()));
  }
...
xmlDocPtr CXAdESGenerator::CreateQualifyingProperties(
  ...
  xmlDocPtr doc = xmlNewDoc(reinterpret_cast<const xmlChar*>("1.0"));
  doc->children =
      xmlNewDocNode(doc, nullptr, BAD_CAST "xades:QualifyingProperties", nullptr);
  ...
  xmlNodePtr pSignedProperties =
      xmlNewChild(pQualifyingProperties, nullptr,
  ...
  xmlNodePtr pSignedSignatureProperties =
      xmlNewChild(pSignedProperties, nullptr,
                  reinterpret_cast<const xmlChar*>("xades:SignedSignatureProperties"), nullptr);
  ...
  xmlNodePtr pSigningCertificate =
      xmlNewChild(pSignedSignatureProperties, nullptr,
                  reinterpret_cast<const xmlChar*>("xades:SigningCertificate"), nullptr);

  // Cert
  xmlNodePtr pCert = xmlNewChild(pSigningCertificate, nullptr,
                                 reinterpret_cast<const xmlChar*>("xades:Cert"), nullptr);

  // CertDigest
  xmlNodePtr pCertDigest =
      xmlNewChild(pCert, nullptr, reinterpret_cast<const xmlChar*>("xades:CertDigest"), nullptr);

  // DigestMethod
  xmlNodePtr pDigestMethod =
      xmlNewChild(pCertDigest, nullptr, reinterpret_cast<const xmlChar*>("ds:DigestMethod"), nullptr);
  ...
  ByteArray baCert2(certval.getContent(), certval.getLength());
  ByteDynArray sha256cert = CSHA256().Digest(baCert2);
  UUCByteArray hashaux(sha256cert.data(), 32);

  std::string strHashB64;
  {
    ByteArray baHashArg(hashaux.getContent(), hashaux.getLength());
  ...
  UUCByteArray* pSerialNumber = const_cast<UUCByteArray*>(serialNumber.getValue());
