...
void CCertStore::AddCertificate(CCertificate& certificate) {
  ...
  try {
    ...
    if (subjectKeyIdentifier.getLength() > 0) {
      UUCByteArray* pValue = const_cast<UUCByteArray*>(subjectKeyIdentifier.getValue());
      ...
      nHash = getHash(reinterpret_cast<const char*>(baSubject.getContent()));
    }
...
CCertificate* CCertStore::GetCertificate(CCertificate& certificate) {
  ...
  try {
    ...
    if (autorityKeyIdentifier.getLength() > 0) {
      UUCByteArray* pValue = const_cast<UUCByteArray*>(autorityKeyIdentifier.getValue());
      pValue->set(0, 0x04);
      ...
      nHash = getHash(reinterpret_cast<const char*>(baIssuer.getContent()));
    }
