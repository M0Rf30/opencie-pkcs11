...
#include "counter_signature_generator.h"
...
void CounterSignatureGenerator::setSigningCertificate(const BYTE* certificate,
                                                      ...
                                                      const BYTE* certHash,
                                                      int certHashLen) {
  m_signingCertificate.append(ByteArray(certificate, certlen));
  m_signerInfoGenerator.setSigningCertificate(certificate, certlen, certHash,
                                              certHashLen);
}

void CounterSignatureGenerator::getContent(ByteDynArray& content) {
  CASN1OctetString signatureValue(m_signerInfo.getEncryptedDigest());
  const ByteDynArray* pValue = signatureValue.getValue();

  content.append(ByteArray(pValue->data(), pValue->size()));
}
...
void CounterSignatureGenerator::getSignedAttributes(
    ByteDynArray& signedAttribute) {
  m_signerInfoGenerator.getSignedAttributes(signedAttribute, true, true);
}

void CounterSignatureGenerator::toByteArray(ByteDynArray& pkcs7SignedData) {
