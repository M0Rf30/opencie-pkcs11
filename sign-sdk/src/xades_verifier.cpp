...
void CXAdESVerifier::parseSignatureNode(xmlXPathContextPtr xpathCtx,
  ...
  *pXAdESDoc->ppSignatures = new SignatureInfo[pXAdESDoc->nSignatures];
  ...
  for (int i = 0; i < signatureNodes->nodeNr; i++) {
    ...
    // SignatureMethod
    snprintf(szPath, sizeof(szPath), "//ds:Signature[@Id='%s']/ds:SignedInfo/ds:SignatureMethod",
            id);
    path = BAD_CAST szPath;
    ...
    if (xpathObj) {
      if (xpathObj->nodesetval->nodeNr > 0) {
        ...
        char* szContent = reinterpret_cast<char*>(signatureMethodNode->properties[0]
                              .children[0]
    ...
    // References
    snprintf(szPath, sizeof(szPath), "//ds:Signature[@Id='%s']/ds:SignedInfo/ds:Reference", id);
    path = BAD_CAST szPath;
    ...
    // SignatureValue
    snprintf(szPath, sizeof(szPath), "//ds:Signature[@Id='%s']/ds:SignatureValue", id);
    path = BAD_CAST szPath;
    ...
    if (xpathObj) {
      if (xpathObj->nodesetval->nodeNr > 0) {
        ...
        char* szContent = reinterpret_cast<char*>(xmlNodeGetContent(signatureValueNode));
        (*pXAdESDoc->ppSignatures[i])
            .sigValue.append(reinterpret_cast<BYTE*>(szContent), strlen(szContent));
      }
    ...
    if (xpathObj) {
      if (xpathObj->nodesetval->nodeNr > 0) {
        ...
        char* szContent =
            reinterpret_cast<char*>(xmlNodeGetContent(certificateNodeSet->nodeTab[0]));
        UUCByteArray content(reinterpret_cast<BYTE*>(szContent), strlen(szContent));
