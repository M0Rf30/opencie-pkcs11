...
Request a standalone RFC 3161 timestamp for a file. Does not require a CIE card.
Computes the SHA-256 digest of `inFilePath` and sends a `TimeStampRequest` to `tsaUrl`.

```c
CK_RV cie_timestamp(const char* inFilePath,
                    const char* tsaUrl,
...
| Parameter | Description |
|-----------|-------------|
| `inFilePath` | Path to the file to timestamp |
| `tsaUrl` | TSA server endpoint URL |
| `tsaUsername` | HTTP Basic auth username; may be `NULL` |
| `tsaPassword` | HTTP Basic auth password; may be `NULL` |
| `outTokenPath` | Path where the DER-encoded `.tst` token is written |
| `progressCallBack` | Progress callback; must not be `NULL` |
